Last login: Mon May 11 23:35:20 on ttys001
andreystoev@Andreys-MacBook-Pro ~ % vi main.c
...
andreystoev@Andreys-MacBook-Pro ~ % clear
...
andreystoev@Andreys-MacBook-Pro ~ % cat main.c
// github.com/andy489
...
int main(int argc, char** argv){
	...
	while((read_sz1=read(fd1_idx, &tr1, sizeof(tr1))) == sizeof(tr1)){
		...
		if(ls == -1){
			...
			errno=_errno;
			err(10, "error while lseek %s", f1_dat);
		}
		int read_sz2 = -1;
		while((read_sz2=read(fd1_dat, &buf, sizeof(buf)) > 0)){
		   if(read_sz2 == -1){
				...
				clo(fd1_dat, fd1_idx, fd2_dat, fd2_idx);
				errno=_errno; 
				err(11, "error while reading %s", f1_dat);
		   }
		   ...
		   if(wr_size == -1){
		      int _errno=errno;
				clo(fd1_dat, fd1_idx, fd2_dat, fd2_idx);
				errno=_errno;
				err(12, "error while writing %s", f2_dat);
		   }
			++len;
			firstLet = 0;
			...
			if(len == tr1.len){
				...
				if(wr_size == -1){
					...
					clo(fd1_dat, fd1_idx, fd2_dat, fd2_idx);
					errno=_errno;
					err(13, "error while writing %s", f2_idx);
				}
				break;
			}	
		}	
	...
	if(read_sz1==-1){
		...
		clo(fd1_dat, fd1_idx, fd2_dat, fd2_idx);
		errno=_errno; 
		err(9, "error while reading %s", f1_idx);
	}
	exit(0);
}
...
andreystoev@Andreys-MacBook-Pro ~ % make main.c
