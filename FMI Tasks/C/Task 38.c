...
int main(int argc, char **argv){
	...
	if(d){
		//printf("we are in case: -d (errors start from 10)\n");
	...
	if(s){
		//printf("we are in case: -s (errors start from 20)\n");
		ssize_t len = strlen(set1);
		ssize_t rd = -1, wr = -1;
		char c, old_c = '\0';
		ssize_t i, flag = 1;
	...
	//printf("we are in case 3: SET1 SET2 (errors start from 30)\n");	
