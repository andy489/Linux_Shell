...
int main(void){
	ssize_t fd = open("foobar.txt", O_RDWR | O_TRUNC | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP/* equiv. to 0644 */);
	if(fd == -1)
	...
	if(wait(NULL) == -1) err_handler(5, "waiting first child", errno, -1); // comment to unorder child processes
