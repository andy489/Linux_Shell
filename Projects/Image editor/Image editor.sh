#!/bin/bash
# Image editor: crop [ipoint faces [options: -m -r]] 
# github.com/andy489
...
function faces {
	...
	# default
        ...
        if [ $(echo "${faces}" | wc -l) -ne 0 ]; then
                ...
                # calculating median of all faces
                read center_x center_y < <(echo "${faces}" \
                        | awk '{coord_x+=$1; coord_y+=$2} END{printf "%.0f %d\n",coord_x/NR, coord_y/NR }')
                        
                elif [ "${flag}" = "r" ]; then
                
                # calculating the center of the smallest rectangle 
                # that covers the set of ALL faces in a pic
                ...
                center_x=$(( smallest_x + (largest_x - smallest_x) / 2 ))
                center_y=$(( smallest_y + (largest_y - smallest_y) / 2 ))
                fi
        fi

        # echo "updated center_x: ${median_x}, updated center_y: ${median_y}"

        ipoint "${a}" "${b}" "${center_x}" "${center_y}" "${input_image}" "${output_image}"
}
